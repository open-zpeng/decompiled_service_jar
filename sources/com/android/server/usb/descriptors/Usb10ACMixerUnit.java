...
public final class Usb10ACMixerUnit extends UsbACMixerUnit {
    ...
    public void report(ReportCanvas canvas) {
        ...
        for (int input = 0; input < numInputs; input++) {
            canvas.write(BackupManagerConstants.DEFAULT_BACKUP_FINISHED_NOTIFICATION_RECEIVERS + ReportCanvas.getHexString(inputIDs[input]));
        ...
        for (int ctrl = 0; ctrl < controls.length; ctrl++) {
            canvas.write(BackupManagerConstants.DEFAULT_BACKUP_FINISHED_NOTIFICATION_RECEIVERS + ((int) controls[ctrl]));

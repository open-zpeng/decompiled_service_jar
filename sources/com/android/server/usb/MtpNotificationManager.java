...
public class MtpNotificationManager {
    ...
    public void showNotification(UsbDevice device) {
        ...
        String title = resources.getString(17041019, device.getProductName());
        String description = resources.getString(17041018);
        Notification.Builder builder = new Notification.Builder(this.mContext, SystemNotificationChannels.USB).setContentTitle(title).setContentText(description).setSmallIcon(17303490).setCategory("sys");
    ...
    private static boolean isMtpDevice(UsbDevice device) {
        for (int i = 0; i < device.getInterfaceCount(); i++) {
            ...
            if (usbInterface.getInterfaceClass() == 255 && usbInterface.getInterfaceSubclass() == 255 && usbInterface.getInterfaceProtocol() == 0 && UsbProfileGroupSettingsManager.UNSUPPORT_IFACE_MTP.equals(usbInterface.getName())) {
                return true;
            }
        }
        return false;

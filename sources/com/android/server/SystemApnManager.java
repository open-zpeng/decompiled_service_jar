...
public final class SystemApnManager {
    ...
    private INetd mNetd = getNetd();

    public SystemApnManager(Context context, INetworkManagementService nms) {
        this.mContext = context;
        this.mNMS = nms;
        if (this.mNetd == null) {
            Slog.wtf(TAG, "No netd service found");
        }
    }
    ...
    public void addSystemApnRoute(String iface, String gateway, String secondIp) {
        ...
        try {
            ...
            InetAddress ia = InetAddress.parseNumericAddress(gateway);
            RouteInfo ri = new RouteInfo(dst, ia, iface);
            this.mNMS.addRoute(14, ri);
        } catch (Exception e) {
            String str2 = TAG;
    ...
    public void addSystemApnNatRule(String iface, String natIpAddr, int mark, int gid) {
        ...
        try {
            this.mNMS.addSystemApnNatRule(iface, mark, natIpAddr, gid);
        } catch (Exception e) {
            String str2 = TAG;
    ...
    public void removeSystemApnNatRule(String iface, String natIpAddr, int mark, int gid) {
        ...
        try {
            this.mNMS.removeSystemApnNatRule(iface, mark, natIpAddr, gid);
        } catch (Exception e) {
    ...
    private List<String> getSystemApnAppPackageNames(File configFile) {
        ...
        try {
            try {
                ...
                do {
                    line = br.readLine();
                    if (line != null && !line.startsWith(AfterSalesDaemonEvent.XP_AFTERSALES_PARAM_SEPARATOR)) {
    ...
    private INetd getNetd() {
        return INetd.Stub.asInterface(ServiceManager.getService("netd"));
    }
}

...
public class MediaSessionRecord implements IBinder.DeathRecipient {
    ...
    private final MediaSession.Token mSessionToken = new MediaSession.Token(this.mController);
    ...
    public void adjustVolume(String packageName, String opPackageName, int pid, int uid, ISessionControllerCallback caller, boolean asSystemService, int direction, int flags, boolean useSuggested) {
        ...
        int flags2 = (isPlaybackActive() || hasFlag(65536)) ? flags & (-5) : flags;
    ...
    private void postAdjustLocalVolume(final int stream, final int direction, final int flags, String callingOpPackageName, int callingPid, int callingUid, boolean asSystemService, final boolean useSuggested, final int previousFlagPlaySound) {
        ...
        this.mHandler.post(new Runnable() { // from class: com.android.server.media.MediaSessionRecord.2
            ...
            public void run() {
                try {
                    if (!useSuggested) {
                        MediaSessionRecord.this.mAudioManagerInternal.adjustStreamVolumeForUid(stream, direction, flags, str, i);
                    } else if (AudioSystem.isStreamActive(stream, 0)) {
                        MediaSessionRecord.this.mAudioManagerInternal.adjustSuggestedStreamVolumeForUid(stream, direction, flags, str, i);
                    } else {
                        MediaSessionRecord.this.mAudioManagerInternal.adjustSuggestedStreamVolumeForUid(Integer.MIN_VALUE, direction, previousFlagPlaySound | flags, str, i);
                    }
                } catch (IllegalArgumentException | SecurityException e) {
    ...
    public void pushQueueUpdate() {
        synchronized (this.mLock) {
            ...
            for (int i = this.mControllerCallbackHolders.size() - 1; i >= 0; i--) {
                ISessionControllerCallbackHolder holder = this.mControllerCallbackHolders.get(i);
                try {
                    holder.mCallback.onQueueChanged(this.mQueue == null ? null : new ParceledListSlice(this.mQueue));
                } catch (DeadObjectException e) {
                    this.mControllerCallbackHolders.remove(i);
    ...
    public MediaController.PlaybackInfo getVolumeAttributes() {
        synchronized (this.mLock) {
            ...
            return new MediaController.PlaybackInfo(volumeType, 2, max, current2, attributes);
        }
    }

...
public class PlatformKeyManager {
    ...
    @VisibleForTesting
    void regenerate(int userId) throws NoSuchAlgorithmException, KeyStoreException, InsecureUserException, IOException, RemoteException {
        int nextId;
    ...
    void init(int userId) throws KeyStoreException, NoSuchAlgorithmException, InsecureUserException, IOException, RemoteException {
        int generationId;
        if (!isAvailable(userId)) {
            throw new InsecureUserException(String.format(Locale.US, "%d does not have a lock screen set.", Integer.valueOf(userId)));
        }
    ...
    private void generateAndLoadKey(int userId, int generationId) throws NoSuchAlgorithmException, KeyStoreException, IOException, RemoteException {
        ...
        KeyProtection.Builder decryptionKeyProtection = new KeyProtection.Builder(2).setUserAuthenticationRequired(true).setUserAuthenticationValidityDurationSeconds(15).setBlockModes("GCM").setEncryptionPaddings("NoPadding");
        if (userId != 0) {
            ...
            decryptionKeyProtection.setBoundToSpecificSecureUserId(secureUserId).setCriticalToDeviceEncryption(true);
        }

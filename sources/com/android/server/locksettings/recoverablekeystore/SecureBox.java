...
public class SecureBox {
    ...
    public static byte[] decrypt(PrivateKey ourPrivateKey, byte[] sharedSecret, byte[] header, byte[] encryptedPayload) throws NoSuchAlgorithmException, InvalidKeyException, AEADBadTagException {
        ...
        if (ourPrivateKey == null) {
            byte[] dhSecret2 = EMPTY_BYTE_ARRAY;
            byte[] hkdfInfo2 = HKDF_INFO_WITHOUT_PUBLIC_KEY;
            hkdfInfo = dhSecret2;
            dhSecret = hkdfInfo2;
        } else {
            byte[] senderPublicKeyBytes = readEncryptedPayload(ciphertextBuffer, 65);
            hkdfInfo = dhComputeSecret(ourPrivateKey, decodePublicKey(senderPublicKeyBytes));
        ...
        byte[] keyingMaterial = concat(hkdfInfo, sharedSecret2);
    ...
    static byte[] concat(byte[]... inputs) {
        int length = 0;
        for (int length2 = 0; length2 < inputs.length; length2++) {
            if (inputs[length2] == null) {
                inputs[length2] = EMPTY_BYTE_ARRAY;
            }
            length += inputs[length2].length;
        }
        byte[] output = new byte[length];
        int outputPos = 0;

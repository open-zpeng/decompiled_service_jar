...
public class DeviceStorageMonitorService extends SystemService {
    ...
    public void check() {
        ...
        for (VolumeInfo vol : storage.getWritablePrivateVolumes()) {
            ...
            if (Math.abs(state.lastUsableBytes - usableBytes) > DEFAULT_LOG_DELTA_BYTES || oldLevel != newLevel) {
                newLevel2 = newLevel;
                int newLevel4 = oldLevel;
                EventLogTags.writeStorageState(uuid.toString(), newLevel4, newLevel2, usableBytes, totalBytes);
    ...
    public void onStart() {
        ...
        if (isTv) {
            this.mNotifManager.createNotificationChannel(new NotificationChannel(TV_NOTIFICATION_CHANNEL_ID, context.getString(17039871), 4));
        }
    ...
    private void updateNotifications(VolumeInfo vol, int oldLevel, int newLevel) {
        ...
        CharSequence title = context.getText(17040308);
        if (StorageManager.UUID_DEFAULT.equals(uuid)) {
            details = context.getText(isBootImageOnDisk() ? 17040306 : 17040307);
        } else {
            CharSequence details2 = context.getText(17040306);
            details = details2;
        }
        CharSequence details3 = details;
        PendingIntent intent = PendingIntent.getActivityAsUser(context, 0, lowMemIntent, 0, null, UserHandle.CURRENT);
        Notification notification = new Notification.Builder(context, SystemNotificationChannels.ALERTS).setSmallIcon(17303517).setTicker(title).setColor(context.getColor(17170460)).setContentTitle(title).setContentText(details3).setContentIntent(intent).setStyle(new Notification.BigTextStyle().bigText(details3)).setVisibility(1).setCategory("sys").extend(new Notification.TvExtender().setChannelId(TV_NOTIFICATION_CHANNEL_ID)).build();
    ...
    private void updateBroadcasts(VolumeInfo vol, int oldLevel, int newLevel, int seq) {
        ...
        if (file.getUsableSpace() < (file.getTotalSpace() * 10) / 100) {
            if (NEW_DEFAULT_NOT_LOW) {
                ...
                intentServer.setPackage("com.xiaopeng.cardiagnosis");

...
public class OtaDexoptService extends IOtaDexopt.Stub {
    ...
    public synchronized void prepare() throws RemoteException {
        ...
        try {
            PackageParser.Package lastUsed = (PackageParser.Package) Collections.max(important, new Comparator() { // from class: com.android.server.pm.-$$Lambda$OtaDexoptService$ZaCsBw0Yn3yN1RRrIRZV-KyDrWE
            ...
            Log.d(TAG, "A/B OTA: lastUsed time = " + lastUsed.getLatestForegroundPackageUseTimeInMills());
    ...
    private synchronized List<String> generatePackageDexopts(PackageParser.Package pkg, int compilationReason) {
        ...
        optimizer.performDexOpt(pkg, null, null, this.mPackageManagerService.getDexManager().getPackageUseInfoOrDefault(pkg.packageName), new DexoptOptions(pkg.packageName, compilationReason, 4));
        return commands;
    }
                ...
                if (pkg.codePath == null) {
                    ...
                    while (i < length) {
                        ...
                        for (String path : paths) {
                            ...
                            Collection<PackageParser.Package> pkgs2 = pkgs;
                            try {

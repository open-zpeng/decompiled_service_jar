...
public class PackageSignatures {
    ...
    private void writeCertsListXml(XmlSerializer serializer, ArrayList<Signature> writtenSignatures, Signature[] signatures, int[] flags) throws IOException {
        for (int i = 0; i < signatures.length; i++) {
            ...
            if (flags != null) {
                serializer.attribute(null, xpInputManagerService.InputPolicyKey.KEY_FLAGS, Integer.toString(flags[i]));
            }
        ...
        while (true) {
            ...
            if (type != 3 && type != 4) {
                ...
                if (tagName.equals("cert")) {
                    if (pos < count) {
                        String index = parser.getAttributeValue(null, AssistDataRequester.KEY_RECEIVER_EXTRA_INDEX);
                        if (index != null) {
                            try {
                            ...
                            if (flags != null) {
                                String flagsStr = parser.getAttributeValue(null, xpInputManagerService.InputPolicyKey.KEY_FLAGS);
                                if (flagsStr != null) {
                                    try {
                    ...
                    String countStr2 = parser.getAttributeValue(null, AssistDataRequester.KEY_RECEIVER_EXTRA_COUNT);
                    if (countStr2 == null) {
                        c = 5;
                    ...
                    try {
                        ...
                        countStr = countStr2;
                        try {
                            int pastSigsPos = readCertsListXml(parser, arrayList2, pastSignatures, pastSignaturesFlags, builder2);
                            builder2 = builder2.setPastSigningCertificates(pastSignatures).setPastSigningCertificatesFlags(pastSignaturesFlags);

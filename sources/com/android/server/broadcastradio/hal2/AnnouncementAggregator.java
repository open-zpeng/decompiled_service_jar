...
public class AnnouncementAggregator extends ICloseHandle.Stub {
    ...
    public void onListUpdated() {
        synchronized (this.mLock) {
            ...
            final List<Announcement> combined = new ArrayList<>();
            for (ModuleWatcher watcher : this.mModuleWatchers) {
                combined.addAll(watcher.currentList);
            }
            TunerCallback.dispatch(new TunerCallback.RunnableThrowingRemoteException() { // from class: com.android.server.broadcastradio.hal2.-$$Lambda$AnnouncementAggregator$93ommYMMUp8HMTemNRAZV3ajfR0
                @Override // com.android.server.broadcastradio.hal2.TunerCallback.RunnableThrowingRemoteException
                public final void run() {
                    AnnouncementAggregator.this.mListener.onListUpdated(combined);
                }
            });
        }
    }

...
public class NetworkTimeUpdateService extends Binder {
    ...
    private static final long TIME_GPS_INTERVAL = 300000;
    private static final long TIME_NTP_GPS_ACCURACY = 300000;
    private static final long TIME_REFRESH_INTERVAL = 120000;
    ...
    public NetworkTimeUpdateService(Context context) {
        ...
        this.mPollingIntervalMs = this.mContext.getResources().getInteger(17694842);
        this.mPollingIntervalShorterMs = this.mContext.getResources().getInteger(17694843);
    ...
    private void onPollNetworkTimeUnderWakeLock(int event) {
        ...
        if (this.mTime.getCacheAge() < this.mPollingIntervalMs) {
            ...
            if (isAutomaticTimeRequested()) {
                boolean tboxSetTime = "1".equals(SystemProperties.get("sys.xiaopeng.usetboxtime", "0"));
                long skew = Math.abs(this.mTime.currentTimeMillis() - System.currentTimeMillis());
                Slog.i(TAG, "tboxSetTime: " + tboxSetTime + ", skew: " + skew);
                String gpsTime = SystemProperties.get("sys.gps.sync.time", "0");
                if (!"0".equals(gpsTime) && Math.abs(this.mTime.currentTimeMillis() - (Long.valueOf(gpsTime).longValue() * 1000)) <= 600000) {
                    Log.d(TAG, "Ignoring NTP update due to recent GPS time");
                    return;
                } else if (!tboxSetTime || skew > 120000) {
    ...
    private void updateSystemClock(int event) {
        ...
        Log.i(TAG, "updateSystemClock timemillis=" + this.mTime.currentTimeMillis());

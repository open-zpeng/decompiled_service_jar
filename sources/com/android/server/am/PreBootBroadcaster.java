...
public abstract class PreBootBroadcaster extends IIntentReceiver.Stub {
    ...
    private Handler mHandler = new Handler(UiThread.get().getLooper(), null, true) { // from class: com.android.server.am.PreBootBroadcaster.1
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 1:
                    CharSequence title = context.getText(17039477);
                    ...
                    Notification notif = new Notification.Builder(PreBootBroadcaster.this.mService.mContext, SystemNotificationChannels.UPDATES).setSmallIcon(17303459).setWhen(0L).setOngoing(true).setTicker(title).setColor(context.getColor(17170861)).setContentTitle(title).setContentIntent(contentIntent).setVisibility(1).setProgress(max, index, false).build();
    ...
    public PreBootBroadcaster(ActivityManagerService service, int userId, ProgressReporter progress, boolean quiet) {
        ...
        this.mTargets = this.mService.mContext.getPackageManager().queryBroadcastReceiversAsUser(this.mIntent, 1048576, UserHandle.of(userId));
    }
    ...
    public void sendNext() {
        if (this.mIndex < this.mTargets.size()) {
            ...
            if (this.mProgress != null) {
                ...
                this.mProgress.setProgress(this.mIndex, this.mTargets.size(), this.mService.mContext.getString(17039471, label));
            }

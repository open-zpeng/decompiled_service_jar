...
public class UserSwitchingDialog extends AlertDialog implements ViewTreeObserver.OnWindowShownListener {
    ...
    void inflateContent() {
        ...
        View view = LayoutInflater.from(getContext()).inflate(17367336, (ViewGroup) null);
        String viewMessage2 = null;
        if (UserManager.isSplitSystemUser() && this.mNewUser.id == 0) {
            viewMessage = res.getString(17041200, this.mOldUser.name);
        } else if (UserManager.isDeviceInDemoMode(this.mContext)) {
            if (this.mOldUser.isDemo()) {
                viewMessage = res.getString(17039865);
            } else {
                viewMessage = res.getString(17039866);
            }
        } else {
            ...
            if (viewMessage2 != null) {
                ...
                viewMessage = res.getString(17041203, this.mNewUser.name);
            }
        }

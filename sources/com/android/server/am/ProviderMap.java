    ...
    */
    ...
    public boolean collectPackageProvidersLocked(String packageName, Set<String> filterByClasses, boolean doit, boolean evenPersistent, int userId, ArrayList<ContentProviderRecord> result) {
        ...
        if (userId == -1) {
            ...
            while (true) {
                int i2 = i;
                if (i2 < this.mProvidersByClassPerUser.size()) {
                    if (collectPackageProvidersLocked(packageName, filterByClasses, doit, evenPersistent, this.mProvidersByClassPerUser.valueAt(i2), result)) {
                        if (!doit) {
                            return true;
                        }
                        didSomething = true;
    ...
    public boolean dumpProvider(FileDescriptor fd, PrintWriter pw, String name, String[] args, int opti, boolean dumpAll) {
        ...
        while (true) {
            int i2 = i;
            int i3 = providers.size();
            if (i2 < i3) {
                if (needSep) {
                    pw.println();
                }
                dumpProvider(BackupManagerConstants.DEFAULT_BACKUP_FINISHED_NOTIFICATION_RECEIVERS, fd, pw, providers.get(i2), args, dumpAll);
                i = i2 + 1;
                needSep = true;
            } else {
                return true;
    ...
    public boolean dumpProviderProto(FileDescriptor fd, PrintWriter pw, String name, String[] args) {
        ...
        while (true) {
            ...
            ContentProviderRecord r = providers.get(i2);
            if (r.proc == null || r.proc.thread == null) {
                i = i2 + 1;
            } else {

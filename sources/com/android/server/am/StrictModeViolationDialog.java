...
final class StrictModeViolationDialog extends BaseErrorDialog {
    ...
    public StrictModeViolationDialog(Context context, ActivityManagerService service, AppErrorResult result, ProcessRecord app) {
        ...
        if (app.pkgList.size() == 1 && (name = context.getPackageManager().getApplicationLabel(app.info)) != null) {
            setMessage(res.getString(17040914, name.toString(), app.info.processName));
        } else {
            setMessage(res.getString(17040915, app.processName.toString()));
        }
        setCancelable(false);
        setButton(-1, res.getText(17039820), this.mHandler.obtainMessage(0));
        if (app.errorReportReceiver != null) {
            setButton(-2, res.getText(17040781), this.mHandler.obtainMessage(1));
        }

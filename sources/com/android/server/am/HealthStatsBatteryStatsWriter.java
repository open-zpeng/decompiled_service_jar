...
public class HealthStatsBatteryStatsWriter {
    ...
    public void writeUid(HealthStatsWriter uidWriter, BatteryStats bs, BatteryStats.Uid uid) {
        ...
        if (controller2 != null) {
            uidWriter.addMeasurement(10020, controller2.getIdleTimeCounter().getCountLocked(2));
            uidWriter.addMeasurement(10021, controller2.getRxTimeCounter().getCountLocked(2));
            long sum2 = 0;
            ...
            uidWriter.addMeasurement(10023, controller2.getPowerCounter().getCountLocked(2));
        }
        BatteryStats.ControllerActivityCounter controller3 = uid.getModemControllerActivity();
        if (controller3 != null) {
            uidWriter.addMeasurement(10024, controller3.getIdleTimeCounter().getCountLocked(2));
            uidWriter.addMeasurement(10025, controller3.getRxTimeCounter().getCountLocked(2));
            long sum3 = 0;
            ...
            uidWriter.addMeasurement(10027, controller3.getPowerCounter().getCountLocked(2));
        }
        ...
        uidWriter.addMeasurement(10045, uid.getUserActivityCount(0, 2));
        uidWriter.addMeasurement(10046, uid.getUserActivityCount(1, 2));
        uidWriter.addMeasurement(10047, uid.getUserActivityCount(2, 2));
    ...
    public void writeProc(HealthStatsWriter procWriter, BatteryStats.Uid.Proc proc) {
        procWriter.addMeasurement((int) EventLogTags.AM_FINISH_ACTIVITY, proc.getUserTime(2));
        procWriter.addMeasurement((int) EventLogTags.AM_TASK_TO_FRONT, proc.getSystemTime(2));
        procWriter.addMeasurement((int) EventLogTags.AM_NEW_INTENT, proc.getStarts(2));
        procWriter.addMeasurement((int) EventLogTags.AM_CREATE_TASK, proc.getNumCrashes(2));
        procWriter.addMeasurement((int) EventLogTags.AM_CREATE_ACTIVITY, proc.getNumAnrs(2));
        procWriter.addMeasurement((int) EventLogTags.AM_RESTART_ACTIVITY, proc.getForegroundTime(2));
    }
    ...
    public void writePkg(HealthStatsWriter pkgWriter, BatteryStats.Uid.Pkg pkg) {
        ...
        for (Map.Entry<String, ? extends BatteryStats.Counter> entry2 : pkg.getWakeupAlarmStats().entrySet()) {
            ...
            if (counter != null) {
                pkgWriter.addMeasurements(40002, entry2.getKey(), counter.getCountLocked(2));
            }
        }
    }

    public void writeServ(HealthStatsWriter servWriter, BatteryStats.Uid.Pkg.Serv serv) {
        servWriter.addMeasurement(50001, serv.getStarts(2));
        servWriter.addMeasurement(50002, serv.getLaunches(2));
    }

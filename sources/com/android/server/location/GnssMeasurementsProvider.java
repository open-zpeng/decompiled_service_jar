...
public abstract class GnssMeasurementsProvider extends RemoteListenerHelper<IGnssMeasurementsListener> {
    ...
    public void onMeasurementsAvailable(final GnssMeasurementsEvent event) {
        RemoteListenerHelper.ListenerOperation<IGnssMeasurementsListener> operation = new RemoteListenerHelper.ListenerOperation() { // from class: com.android.server.location.-$$Lambda$GnssMeasurementsProvider$865xzodmeiSeR2xhh7cKZjiZkhE
            ...
            public final void execute(IInterface iInterface) {
                ((IGnssMeasurementsListener) iInterface).onGnssMeasurementsReceived(event);
            }
        };
        foreach(operation);
    }

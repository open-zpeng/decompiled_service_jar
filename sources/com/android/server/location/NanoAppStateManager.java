...
public class NanoAppStateManager {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public synchronized Collection<NanoAppInstanceInfo> getNanoAppInstanceInfoCollection() {
    ...
    public synchronized void addNanoAppInstance(int contextHubId, long nanoAppId, int nanoAppVersion) {
        ...
        int i = 0;
        int nanoAppHandle2 = nanoAppHandle;
        int nanoAppHandle3 = 0;
        while (true) {
            if (nanoAppHandle3 > Integer.MAX_VALUE) {
                break;
            } else if (!this.mNanoAppHash.containsKey(Integer.valueOf(nanoAppHandle2))) {
                this.mNanoAppHash.put(Integer.valueOf(nanoAppHandle2), new NanoAppInstanceInfo(nanoAppHandle2, nanoAppId, nanoAppVersion, contextHubId));
                if (nanoAppHandle2 != Integer.MAX_VALUE) {
                    i = nanoAppHandle2 + 1;
                }
                this.mNextHandle = i;
            } else {
                nanoAppHandle2 = nanoAppHandle2 == Integer.MAX_VALUE ? 0 : nanoAppHandle2 + 1;
                nanoAppHandle3++;
            }
        }
        Log.v(TAG, "Added app instance with handle " + nanoAppHandle2 + " to hub " + contextHubId + ": ID=0x" + Long.toHexString(nanoAppId) + ", version=0x" + Integer.toHexString(nanoAppVersion));
    }
    ...
    public synchronized void removeNanoAppInstance(int contextHubId, long nanoAppId) {
        ...
        this.mNanoAppHash.remove(Integer.valueOf(nanoAppHandle));
    }
    ...
    private void handleQueryAppEntry(int contextHubId, long nanoAppId, int nanoAppVersion) {
        ...
        NanoAppInstanceInfo info = this.mNanoAppHash.get(Integer.valueOf(nanoAppHandle));

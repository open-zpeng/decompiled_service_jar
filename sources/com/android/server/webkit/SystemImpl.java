...
public class SystemImpl implements SystemInterface {
    ...
    private SystemImpl() {
        ...
        try {
            try {
                XmlResourceParser parser2 = AppGlobals.getInitialApplication().getResources().getXml(18284550);
                XmlUtils.beginDocument(parser2, TAG_START);
                while (true) {
                    XmlUtils.nextElement(parser2);
                    String element = parser2.getName();
                    if (element != null) {
                        if (element.equals(TAG_WEBVIEW_PROVIDER)) {
                            String packageName = parser2.getAttributeValue(null, "packageName");
                            if (packageName == null) {
                                throw new AndroidRuntimeException("WebView provider in framework resources missing package name");
                            }
                            String description = parser2.getAttributeValue(null, TAG_DESCRIPTION);
                            if (description == null) {
                                throw new AndroidRuntimeException("WebView provider in framework resources missing description");
                            }
                            boolean availableByDefault = "true".equals(parser2.getAttributeValue(null, TAG_AVAILABILITY));
                            boolean isFallback = "true".equals(parser2.getAttributeValue(null, TAG_FALLBACK));
                            WebViewProviderInfo currentProvider = new WebViewProviderInfo(packageName, description, availableByDefault, isFallback, readSignatures(parser2));
    ...
    public long getFactoryPackageVersion(String packageName) throws PackageManager.NameNotFoundException {
        ...
        return pm.getPackageInfo(packageName, DumpState.DUMP_COMPILER_STATS).getLongVersionCode();
    }

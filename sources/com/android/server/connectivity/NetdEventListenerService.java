...
public class NetdEventListenerService extends INetdEventListener.Stub {
    ...
    @Override // android.net.metrics.INetdEventListener
    public synchronized void onTcpSocketStatsEvent(int[] networkIds, int[] sentPackets, int[] lostPackets, int[] rttsUs, int[] sentAckDiffsMs) throws RemoteException {
        int i;
        if (networkIds.length == sentPackets.length && networkIds.length == lostPackets.length && networkIds.length == rttsUs.length && networkIds.length == sentAckDiffsMs.length) {
            long timestamp = System.currentTimeMillis();
            for (int i2 = 0; i2 < networkIds.length; i2++) {
                int netId = networkIds[i2];
                int sent = sentPackets[i2];
                int lost = lostPackets[i2];
                int rttUs = rttsUs[i2];
                int sentAckDiffMs = sentAckDiffsMs[i2];
            ...
            int i3 = 0;
            while (i3 < length) {
                INetdEventCallback callback = iNetdEventCallbackArr[i3];
                if (callback == null || networkIds.length <= 0) {
                    i = i3;
                } else {
                    i = i3;
                    callback.onTcpStatsEvent(networkIds, sentPackets, lostPackets, rttsUs, sentAckDiffsMs);
                }
                i3 = i + 1;
            }
            return;

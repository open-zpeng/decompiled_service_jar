...
public class UpstreamNetworkMonitor {
    ...
    public NetworkState getCurrentPreferredUpstream() {
        ...
        if (this.mDefaultInternetNetwork != null) {
            dfltState = this.mNetworkMap.get(this.mDefaultInternetNetwork);
        } else {
            dfltState = null;
        }
        return (this.mDunRequired && !isNetworkUsableAndNotCellular(dfltState)) ? findFirstDunNetwork(this.mNetworkMap.values()) : dfltState;
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void handleNetCap(int callbackType, Network network, NetworkCapabilities newNc) {
        if (callbackType == 2) {
    ...
    public void handleLost(int callbackType, Network network) {
        if (network.equals(this.mDefaultInternetNetwork)) {
            this.mDefaultInternetNetwork = null;
            if (callbackType == 2) {
                return;
            }
    ...
    public class UpstreamNetworkCallback extends ConnectivityManager.NetworkCallback {
        ...
        public void onCapabilitiesChanged(Network network, NetworkCapabilities newNc) {
            UpstreamNetworkMonitor.this.handleNetCap(this.mCallbackType, network, newNc);
        }

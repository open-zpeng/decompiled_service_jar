...
public class AlertWindowNotification {
    ...
    public void cancel(final boolean deleteChannel) {
        this.mService.mH.post(new Runnable() { // from class: com.android.server.wm.-$$Lambda$AlertWindowNotification$ZuqSYML-X-nkNVTba_yeIT9hJ1s
            @Override // java.lang.Runnable
            public final void run() {
                AlertWindowNotification.this.onCancelNotification(deleteChannel);
            }
        });
    }
    ...
    public void onPostNotification() {
        ...
        Notification.Builder builder = new Notification.Builder(context, this.mNotificationTag).setOngoing(true).setContentTitle(context.getString(17039466, appName)).setContentText(message).setSmallIcon(17301708).setColor(context.getColor(17170861)).setStyle(new Notification.BigTextStyle().bigText(message)).setLocalOnly(true).addExtras(extras).setContentIntent(getContentIntent(context, this.mPackageName));

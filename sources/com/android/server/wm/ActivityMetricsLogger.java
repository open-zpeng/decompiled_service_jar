...
public class ActivityMetricsLogger {
    ...
    public void checkVisibility(TaskRecord t, ActivityRecord r) {
        synchronized (this.mSupervisor.mService.mGlobalLock) {
            try {
                ...
                } else if (info.launchedActivity != r) {
                    WindowManagerService.resetPriorityAfterLockedSection();
    ...
    private void logAppTransitionMultiEvents() {
        ...
        for (int index = this.mWindowingModeTransitionInfo.size() - 1; index >= 0; index--) {
            ...
            BackgroundThread.getHandler().post(new Runnable() { // from class: com.android.server.wm.-$$Lambda$ActivityMetricsLogger$0lYHhsj1ibZMP6eUAxlwT_URn-4
                @Override // java.lang.Runnable
                public final void run() {
                    ActivityMetricsLogger.this.lambda$logAppTransitionMultiEvents$0$ActivityMetricsLogger(currentTransitionDeviceUptime, currentTransitionDelayMs, infoSnapshot);
                }
            });
            BackgroundThread.getHandler().post(new Runnable() { // from class: com.android.server.wm.-$$Lambda$ActivityMetricsLogger$DGh4oiUM74EvEwFf0Up59dk2u_I
                @Override // java.lang.Runnable
                public final void run() {
                    ActivityMetricsLogger.this.lambda$logAppTransitionMultiEvents$1$ActivityMetricsLogger(infoSnapshot);
                }
            });
    ...
    public WindowingModeTransitionInfoSnapshot logAppTransitionReportedDrawn(ActivityRecord r, boolean restoredFromBundle) {
        ...
        Trace.traceBegin(64L, "ActivityManager:ReportingFullyDrawn " + info.launchedActivity.packageName);
        ...
        BackgroundThread.getHandler().post(new Runnable() { // from class: com.android.server.wm.-$$Lambda$ActivityMetricsLogger$xxgDrxthXaC_M25gC-YvHOsQ6yE
            @Override // java.lang.Runnable
            public final void run() {
                ActivityMetricsLogger.this.lambda$logAppTransitionReportedDrawn$2$ActivityMetricsLogger(infoSnapshot);
            }
        });
        return infoSnapshot;
    }
    ...
    public void logAbortedBgActivityStart(Intent intent, WindowProcessController callerApp, int callingUid, String callingPackage, int callingUidProcState, boolean callingUidHasAnyVisibleWindow, int realCallingUid, int realCallingUidProcState, boolean realCallingUidHasAnyVisibleWindow, boolean comingFromPendingIntent) {
        ...
        if (callerApp != null) {
            ...
            if (callerApp.getWhenUnimportant() != 0) {
                builder.addTaggedData((int) UsbTerminalTypes.TERMINAL_EXTERN_LINE, Long.valueOf(nowUptime - callerApp.getWhenUnimportant()));
            }
        }
        this.mMetricsLogger.write(builder);
    }
    ...
    private void launchObserverNotifyActivityLaunched(WindowingModeTransitionInfo info) {
        ...
        int temperature = convertTransitionTypeToLaunchObserverTemperature(getTransitionType(info));
        this.mLaunchObserver.onActivityLaunched(convertActivityRecordToProto(info.launchedActivity), temperature);
        Trace.traceEnd(64L);
    }
    ...
    private void launchObserverNotifyActivityLaunchCancelled(WindowingModeTransitionInfo info) {
        ...
        byte[] activityRecordProto = info != null ? convertActivityRecordToProto(info.launchedActivity) : null;

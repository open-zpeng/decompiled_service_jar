...
public class xpBootManagerPolicy {
    ...
    private static final double CPU = xpTextUtils.toDouble(SystemProperties.get("persist.xp.boot.policy.cpu"), Double.valueOf((double) sLimitedCpu)).doubleValue();
    ...
    private Handler mHandler = new Handler(Looper.getMainLooper()) { // from class: com.xiaopeng.server.policy.xpBootManagerPolicy.1
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                default:
                    return;
                case xpBootManagerPolicy.MSG_HANDLE_ACTIVITY_CHANGED /* 1002 */:
    ...
    public void onActivityChanged(ComponentName component) {
        this.mHandler.sendMessage(this.mHandler.obtainMessage(MSG_HANDLE_ACTIVITY_CHANGED, component));
    }

    public boolean abortActivityIfNeed(Context context, ComponentName component) {
        boolean abort;
        boolean isFallbackHome = false;
        if (BOOT_POLICY_ENABLED && !sBootReady && !sActivityReady && xpActivityManager.isComponentValid(component)) {
            ...
            isFallbackHome = (sFallbackHome == null || isFallbackHome(component)) ? true : true;
            ...
            log(TAG, "abortActivityIfNeed abort=" + abort + " component=" + component + " home=" + isHome + " preload=" + isPreload + " isFallbackHome=" + isFallbackHome + " fallbackHome=" + sFallbackHome);
            return abort;
        }
        return false;
    }
    ...
    */
    ...
    private boolean isFallbackHome(ComponentName component) {
        log(TAG, "isFallbackHome component=" + component + " fallbackHome=" + sFallbackHome);
        return sFallbackHome != null && xpActivityManager.isComponentEqual(sFallbackHome, component);
    }
    ...
    public void onBootCompleted() {
        ...
        this.mHandler.post(new Runnable() { // from class: com.xiaopeng.server.policy.-$$Lambda$xpBootManagerPolicy$rSmnJGqfOlZ9TzpClb6cfZH9R6A
            ...
            public final void run() {
                xpBootManagerPolicy.lambda$onBootCompleted$0(xpBootManagerPolicy.this);
            }
        });
    }

    public static /* synthetic */ void lambda$onBootCompleted$0(xpBootManagerPolicy xpbootmanagerpolicy) {
        try {
            Intent intent = new Intent("com.xiaopeng.intent.action.BOOT_COMPLETED");
            intent.addFlags(16777216);
            xpbootmanagerpolicy.mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
    ...
    private static boolean preloadComponentsNotEmpty() {
        return sPreloadComponents != null && sPreloadComponents.length > 0;
    }

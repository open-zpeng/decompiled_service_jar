...
public class xuiChannelToAmp implements IXuiChannelInterface, remoteConnect.RemoteConnectListerer {
    ...
    static long lastChangeChannelTime = 0;
    ...
    private void initXuiChannelData() {
        ...
        if (this.activeChannelBits == 0) {
            this.activeChannelBits += 8;
    ...
    private int getSpdifChannelBits() {
        ...
        int spdifbits = 0;
        for (int i = 0; i < mStreamVolumeType.getMaxStreamType(); i++) {
            if (this.mActiveStreamList[i] != null && this.mActiveStreamList[i].size() != 0) {
    ...
    private int[] getAllChannelBits() {
        ...
        int spdifbits = 0;
        for (int spdifbits2 = 0; spdifbits2 < mStreamVolumeType.getMaxStreamType(); spdifbits2++) {
            if (this.mActiveStreamList[spdifbits2] != null && this.mActiveStreamList[spdifbits2].size() != 0 && (mTypePolicy = getTypePolicy(getSceenPolicyByStreamType(spdifbits2))) != null) {
                if (mTypePolicy.type.equals("spdif")) {
        ...
        if (i != -1 && this.btCallOnFlag == 0 && this.mMainDriverMode != 2) {
            analogbits = this.mSpeechChannels;
        }
        int spdifbits3 = (~mixbits) & spdifbits;
        int spdifbits4 = ~mixbits;
        int analogbits2 = analogbits & spdifbits4;
        int activeChannelBits = spdifbits3 | analogbits2 | mixbits;
        int spdifChannelBits = (~analogbits2) & spdifbits3;
        int analogChannelBits = (~spdifbits3) & analogbits2;
        int mixbits2 = mixbits | (spdifbits3 & analogbits2);
    ...
    private channelData.ScreenPolicy getSceenPolicyByStreamType(int streamType) {
        ...
        if (streamStr.equals(BackupManagerConstants.DEFAULT_BACKUP_FINISHED_NOTIFICATION_RECEIVERS) || !checkChannelData()) {
    ...
    private boolean checkChannelData() {
        if (mChannelData == null || mChannelData.policy == null) {
            return false;
        }
        return true;
    }
    ...
    private void doSaveActiveChannelBits(int streamType, int id, int playFlag) {
        ...
        debugDumpActiveList(streamType);
    ...
    private void saveStreamTypeStatus(int streamType, int id, int playFlag) {
        if (streamType != 6) {
            ...
            if (streamType <= 10 && mStreamVolumeType != null && streamType < mStreamVolumeType.getMaxStreamType()) {
    ...
    public void igStatusChange(int state) {
        ...
        if (state == 0) {
            ...
            synchronized (this.activeStreamListObject) {
                ...
                this.mActiveStreamList[3].add(Integer.valueOf((int) NetworkConstants.ARP_HWTYPE_RESERVED_HI));
                this.mActiveStreamList[1].add(Integer.valueOf((int) NetworkConstants.ARP_HWTYPE_RESERVED_HI));
            }
            resetActiveBits();
        }
        lastChangeChannelTime = 0L;
    }
    ...
    public void changeChannel() {
        if (this.mXpVehicle == null) {
            Log.i(TAG, "changeChannel()  carService not connected!!");
        } else if (this.mForceSurroundMode) {
            ...
            if (this.lastSpdifbits == spdifbits && this.lastAnalogbits == analogbits && this.lastMixbits == mixbits && this.lastActiveBits == activeBits && Math.abs(currentTime - lastChangeChannelTime) < 3000) {
            ...
            lastChangeChannelTime = currentTime;
            try {
                if (this.isAmpAllChannelOn && this.mXpVehicle != null) {
                    this.mXpVehicle.setAmpChannelVolAndSource(1023, 100, 3, 1023);
                } else if (this.mXpVehicle != null && activeBits != 0) {
                    if (spdifbits != 0) {
    ...
    public void forceChangeToAmpChannel(int channelBits, int activeBits, int volume, boolean stop) {
        ...
        Log.d(TAG, "forceChangeToAmpChannel " + channelBits + " " + activeBits + " " + volume + " " + stop);
        try {
            if (this.mXpVehicle != null) {
    ...
    private void debugDumpActiveList(int streamType) {
        if (this.mActiveStreamList == null || this.mActiveStreamList[streamType] == null) {
            return;
        }
        synchronized (this.activeStreamListObject) {
            for (int i = 0; i < this.mActiveStreamList[streamType].size(); i++) {
                Log.d(TAG, "   " + i + ":  " + this.mActiveStreamList[streamType].get(i));
            }
        }
    }
}

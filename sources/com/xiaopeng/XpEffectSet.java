...
public class XpEffectSet {
    ...
    private void setSpeechEffect(int effectToolIndex) {
        XpEffectParam xpEffectParam;
        if (this.mEffectToolList[effectToolIndex].mEqualizer == null || this.mEffectToolList[effectToolIndex].mBassBoost == null || this.mEffectToolList[effectToolIndex].mPresetReverb == null || (xpEffectParam = this.mXpEffectParam) == null) {
        ...
        if (xpEffectParam.xEqualizerUsePreset == -1) {
    ...
    private int findEffectToolIndex(int audiosessionid) {
        for (int i = 0; i < 3; i++) {
            EffectToolList[] effectToolListArr = this.mEffectToolList;
            if (effectToolListArr[i] == null) {
                effectToolListArr[i] = new EffectToolList();
            }
    ...
    public int findEffectToolUsed() {
        ...
        for (int i = 0; i < 3; i++) {
            EffectToolList[] effectToolListArr = this.mEffectToolList;
            if (effectToolListArr[i] != null && effectToolListArr[i].sessinID != -1) {
                num++;
            }
        }

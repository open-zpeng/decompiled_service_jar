...
public class ApfFilter {
    ...
    private final BroadcastReceiver mDeviceIdleReceiver = new BroadcastReceiver() { // from class: android.net.apf.ApfFilter.1
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("android.os.action.DEVICE_IDLE_MODE_CHANGED")) {
                ...
                ApfFilter.this.setDozeMode(deviceIdle);
            }
        }
    };
    @GuardedBy("this")
        ...
        public void run() {
            ...
            while (!this.mStopped) {
                try {
                    ...
                    updateStats(ApfFilter.this.processRa(this.mPacket, length));
    ...
    private static int[] filterEthTypeBlackList(int[] ethTypeBlackList) {
        ...
        return bl.stream().mapToInt(new ToIntFunction() { // from class: android.net.apf.-$$Lambda$ApfFilter$UV1wDVoVlbcxpr8zevj_aMFtUGw
    ...
    public class Ra {
        ...
        public String toString() {
            try {
                ...
                sb.append(String.format("RA %s -> %s %ds ", IPv6AddresstoString(22), IPv6AddresstoString(38), Integer.valueOf(BitUtils.getUint16(this.mPacket, 60))));
        ...
        Ra(byte[] packet, int length) throws InvalidRaException {
            ...
            int lastNonLifetimeStart = addNonLifetime(0, 15, 3);
            int lastNonLifetimeStart2 = addNonLifetime(addNonLifetime(lastNonLifetimeStart, 56, 2), 60, 2);
            ...
            while (this.mPacket.hasRemaining()) {
                ...
                if (optionType == 3) {
                    int lastNonLifetimeStart3 = addNonLifetime(lastNonLifetimeStart2, 4, 4);
                    long lifetime = BitUtils.getUint32(this.mPacket, position + 4);
                    builder.updatePrefixValidLifetime(lifetime);
                    lastNonLifetimeStart2 = addNonLifetime(lastNonLifetimeStart3, 8, 4);
                    long lifetime2 = BitUtils.getUint32(this.mPacket, position + 8);
                    builder.updatePrefixPreferredLifetime(lifetime2);
                    this.mPrefixOptionOffsets.add(Integer.valueOf(position));
                } else if (optionType != 31) {
                    switch (optionType) {
                        case 24:
                            lastNonLifetimeStart2 = addNonLifetimeU32(lastNonLifetimeStart2);
                            long lifetime3 = BitUtils.getUint32(this.mPacket, position + 4);
                            builder.updateRouteInfoLifetime(lifetime3);
                            break;
                        case 25:
                            this.mRdnssOptionOffsets.add(Integer.valueOf(position));
                            lastNonLifetimeStart2 = addNonLifetimeU32(lastNonLifetimeStart2);
                            long lifetime4 = BitUtils.getUint32(this.mPacket, position + 4);
                            builder.updateRdnssLifetime(lifetime4);
                            break;
                    }
                } else {
                    lastNonLifetimeStart2 = addNonLifetimeU32(lastNonLifetimeStart2);
                    long lifetime5 = BitUtils.getUint32(this.mPacket, position + 4);
                    builder.updateDnsslLifetime(lifetime5);
                }
                if (optionLength <= 0) {
                    throw new InvalidRaException(String.format("Invalid option length opt=%d len=%d", Integer.valueOf(optionType), Integer.valueOf(optionLength)));
                }
    ...
    private void generateIPv4FilterLocked(ApfGenerator gen) throws ApfGenerator.IllegalInstructionException {
        if (this.mMulticastFilter) {
            ...
            if (this.mIPv4Address != null && this.mIPv4PrefixLength < 31) {
                ...
                int broadcastAddr = ipv4BroadcastAddress(this.mIPv4Address, this.mIPv4PrefixLength);
    ...
    public static ApfFilter maybeCreate(Context context, ApfConfiguration config, InterfaceParams ifParams, IpClient.Callback ipClientCallback) {
        ...
        if (context == null || config == null || ifParams == null || (apfCapabilities = config.apfCapabilities) == null || apfCapabilities.apfVersionSupported == 0) {
            return null;
        }
    ...
    public synchronized void dump(IndentingPrintWriter pw) {
        ...
        pw.println(String.format("Last program length %d, installed %ds ago, lifetime %ds", Integer.valueOf(this.mLastInstalledProgram.length), Long.valueOf(currentTimeSeconds() - this.mLastTimeInstalledProgram), Long.valueOf(this.mLastInstalledProgramMinLifetime)));
        ...
        while (it.hasNext()) {
            ...
            pw.println(ra);
            pw.increaseIndent();
            pw.println(String.format("Seen: %d, last %ds ago", Integer.valueOf(ra.seenCount), Long.valueOf(currentTimeSeconds() - ra.mLastSeen)));
